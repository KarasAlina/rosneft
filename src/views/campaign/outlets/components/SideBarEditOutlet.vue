<template>
  <b-sidebar
      :visible="isActiveSideBarEditOutlet"
      bg-variant="white"
      ...
      @hidden="resetForm"
      @shown="resolveFields"
      @change="(val) => $emit('update:is-active-side-bar-edit-outlet', val)"
  >
...
export default {
  name: 'SideBarEditOutlet',

  model: {
    prop: 'isActiveSideBarEditOutlet',
    event: 'update:is-active-side-bar-edit-outlet',
  },

  props: {
    isActiveSideBarEditOutlet: {
      type: Boolean,
      required: true,
    },
    ...
    async send() {
      if (!this.submitStatus) {
        ...
        try {
          ...
          this.$emit('update:is-active-side-bar-edit-outlet', false);
        } catch (e) {
          console.log('--- ', e);
        } finally {
