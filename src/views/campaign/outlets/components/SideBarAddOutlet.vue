<template>
  <b-sidebar
      :visible="isActiveSideBarAddOutlet"
      bg-variant="white"
      ...
      right
      @hidden="resetForm"
      @change="(val) => $emit('update:is-active-side-bar-add-outlet', val)"
  >
...
export default {
  name: 'SideBarAddOutlet',

  model: {
    prop: 'isActiveSideBarAddOutlet',
    event: 'update:is-active-side-bar-add-outlet',
  },

  props: {
    isActiveSideBarAddOutlet: {
      type: Boolean,
      required: true,
    },
  },
  ...
  methods: {
    ...
    async send() {
      if (!this.submitStatus) {
        ...
        try {
          await this.$store.dispatch('outlet/CreateOutlet', data);

          this.resetForm();

          this.$emit('refetch-data');

          this.$emit('update:is-active-side-bar-add-outlet', false);
        } catch (e) {
          console.log('--- ', e);
        } finally {
    ...
    resolveFields() {
      const a = (this.$store.getters['outlet/options'])
