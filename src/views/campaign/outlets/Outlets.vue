  ...
  <b-card no-body>
    ...
    <template v-if="outlets">
      ...
      <SideBarViewOutlet
          ...
          :isActiveSideBarViewOutlet.sync="isActiveSideBarViewOutlet"/>

      <SideBarAddOutlet
          @refetch-data="getOutlets"
          :isActiveSideBarAddOutlet.sync="isActiveSideBarAddOutlet"/>

      <SideBarEditOutlet
          :moduleId="currentProgram.name"
          :current="currentOutlet"
          @refetch-data="getOutlets"
          :isActiveSideBarEditOutlet.sync="isActiveSideBarEditOutlet"/>
    </template>
  </b-card>
</div>
</template>
...
export default {
  ...
  computed: {
    ...
    totalCount() {
      ...
      return this.$store.getters['outlet/list']?._meta.totalCount;
    },
    totalVisibleFields() {
  ...
  methods: {
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('outlet/SET_OPTIONS', this.fields);
    ...
    async getOutlets() { // все коды текущего промо
      this.pending = true;
      ...
      await this.$store.dispatch('outlet/GetOutlet', o);

      this.pending = null;
    },

    async getOptions(moduleId) { // получить список полей
      await this.$store.dispatch('outlet/GetOptions', {
        moduleId,
      });
    },
    ...
    async deleteOutlet(id, moduleId) {
      this.pending = true;
  ...
  components: {
    ...
    SideBarAddOutlet: () => import('./components/SideBarAddOutlet.vue'),
    SideBarEditOutlet: () => import('./components/SideBarEditOutlet.vue'),
    SideBarViewOutlet: () => import('./components/SideBarViewOutlet.vue'),
