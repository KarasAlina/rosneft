  ...
  <b-card no-body>
    ...
    <template v-if="codes">
      <div>
        ...
        <!-- Table -->
        <b-table
            @sort-changed="updateSort($event); getCodes()"
            responsive
            :items="codes"
        ...
        >
          ...
          <template #cell(type)="data">
            <div>{{ data.field.data.value[data.value] || '-' }}</div>
          </template>
...
import {
  BCard, BTable, BRow, BCol, BPagination, BDropdown, BDropdownForm, BBadge, BFormCheckbox, BOverlay,
} from 'bootstrap-vue';
...
export default {
  ...
  computed: {
    ...
    fields() {
      return this.$store.getters['code/options'];
    },

    totalCount() {
      /* eslint no-underscore-dangle: ["error", { "allow": ["_meta"] }] */
      return this.$store.getters['code/list']?._meta.totalCount;
    },
    ...
    codes() {
      return this.$store.getters['code/list']?.items;
    },
  ...
  watch: {
    currentProgram() {
      ...
      this.currentPage = 1;
      this.page = 1;
      this.getCodes(this.currentProgram?.name);
    },
  },
  ...
  methods: {
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('code/SET_OPTIONS', this.fields);
      ...
      await this.$store.dispatch('code/SetOption', data);
    ...
    async getCodes() { // все коды текущего промо
      this.pending = true;
      ...
      await this.$store.dispatch('code/GetCodes', o);

      this.pending = null;
    },

    async getOptions(moduleId) { // получить список полей
      await this.$store.dispatch('code/GetOptions', {
        moduleId,
      });
    },
  },
