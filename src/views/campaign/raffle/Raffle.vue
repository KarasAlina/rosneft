  ...
  <b-card no-body>
    ...
    <template v-if="raffle">
      <div>
        ...
        <!-- Table -->
        <b-table
            @sort-changed="updateSort($event); getRaffle()"
            responsive
            :items="raffle"
...
export default {
  ...
  computed: {
    ...
    fields() {
      return this.$store.getters['raffle/options'];
    },

    totalCount() {
      /* eslint no-underscore-dangle: ["error", { "allow": ["_meta"] }] */
      return this.$store.getters['raffle/list']?._meta.totalCount;
    },
    ...
    raffle() {
      return this.$store.getters['raffle/list']?.items;
    },
  ...
  watch: {
    currentProgram() {
      this.currentPage = 1;
      this.page = 1;
      this.getRaffle(this.currentProgram?.name);
    },
  },
  ...
  methods: {
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('raffle/SET_OPTIONS', this.fields);
      ...
      await this.$store.dispatch('raffle/SetOption', data);
    ...
    async getRaffle() { // все розыгрышы текущего промо
      this.pending = true;
      ...
      try {
        await this.$store.dispatch('raffle/GetRaffle', o);
      } catch (e) {
    ...
    async getOptions(moduleId) { // получить список полей
      try {
        await this.$store.dispatch('raffle/GetOptions', {
          moduleId,
        });
      } catch (e) {
        await this.$router.push('/');
