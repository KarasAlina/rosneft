    ...
    <b-card no-body>
      ...
      <template v-if="sale">
        <div>
          ...
          <FlipTable>
            <b-table
                @sort-changed="updateSort($event); getSales()"
                :items="sale"
...
export default {
  ...
  computed: {
    ...
    fields() {
      return this.$store.getters['sale/options'];
    },

    totalCount() {
      /* eslint no-underscore-dangle: ["error", { "allow": ["_meta"] }] */
      return this.$store.getters['sale/list']?._meta.totalCount;
    },
    ...
    sale() {
      return this.$store.getters['sale/list']?.items;
    },
  ...
  watch: {
    currentProgram() {
      this.getSales(this.currentProgram?.name);
    },
  },
  ...
  methods: {
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('sale/SET_OPTIONS', this.fields);
      ...
      await this.$store.dispatch('sale/SetOption', data);
    ...
    async getSales() { // все призы текущего промо
      this.pending = true;
      ...
      try {
        await this.$store.dispatch('sale/GetSales', o);
      } catch (e) {
    ...
    async getOptions(moduleId) { // получить список полей
      try {
        await this.$store.dispatch('sale/GetOptions', {
          moduleId,
        });
      } catch (e) {
        // await this.$router.push('/');
