  ...
  <b-card no-body>
    ...
    <template v-if="prizes">
      <div>
          ...
          <b-row class="mb-50">
            ...
            >
              ...
              <div
                ...
                >
                  <template v-if="item.key === 'prize' || item.key === 'status'">
        ...
        <!-- Table -->
        <b-table
            @sort-changed="updateSort($event); getPrizes()"
            responsive
            :items="prizes"
...
export default {
  ...
  computed: {
    ...
    fields() {
      return this.$store.getters['prize/options'];
    },

    totalCount() {
      /* eslint no-underscore-dangle: ["error", { "allow": ["_meta"] }] */
      return this.$store.getters['prize/list']?._meta.totalCount;
    },
    ...
    prizes() {
      return this.$store.getters['prize/list']?.items;
    },
  ...
  watch: {
    currentProgram() {
      this.currentPage = 1;
      this.page = 1;
      this.getPrizes(this.currentProgram?.name);
    },
  },
  ...
  methods: {
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('prize/SET_OPTIONS', this.fields);
      ...
      await this.$store.dispatch('prize/SetOption', data);
    ...
    async getPrizes() { // все призы текущего промо
      this.pending = true;
      ...
      try {
        await this.$store.dispatch('prize/GetPrizes', o);
      } catch (e) {
    ...
    async getOptions(moduleId) { // получить список полей
      try {
        await this.$store.dispatch('prize/GetOptions', {
          moduleId,
        });
      } catch (e) {
        await this.$router.push('/');
