  ...
  <b-card no-body>
    ...
    <template v-if="profiles">
      <div>
          ...
          <b-row class="mb-50">
            ...
            >
              <div v-if="visibleFields" class="d-flex align-items-center flex-wrap mb-50">
                <!-- Filter -->
                <FilterDrop
                    :options="visibleFields.filter(item => (item.key !== 'actions' && item.key !== 'photos'))"
                    @trigger="resolveFilter($event)"/>
              </div>
              ...
              <div
                ...
                >
                  <template v-if="item.key === 'status'">
                    {{item.label}} = {{ fields.filter((i) => i.key === item.key)[0].data.value[item.value] }}
                  </template>
        ...
        <!-- Table -->
        <b-table
            @sort-changed="updateSort($event); getChecks()"
            responsive
            :items="profiles"
        ...
        >
          ...
          <template #cell(photos)="data">
            <a
                v-if="data.value"
                href="#"
                @click.prevent="openImages(data.value)">
              открыть
            </a>
          ...
          <template #cell(actions)="data">
            ...
            >
              ...
              <b-dropdown-item @click="showSideBarViewCheck(data.item)">
                <feather-icon icon="EyeIcon" />
                <span class="align-middle ml-50">Посмотреть</span>
              </b-dropdown-item>

              <b-dropdown-item @click="showSideBarEditCheck(data.item)">
                <feather-icon icon="EditIcon" />
                <span class="align-middle ml-50">Редактировать</span>
              </b-dropdown-item>

              <b-dropdown-item @click="deleteCheck(data.item.id, currentProgram.name)">
      ...
      <SideBarView
          @openImages="openImages($event)"
          :current="currentCheck"
          :isActiveSideBarViewCheck.sync="isActiveSideBarViewCheck"/>

      <SideBarEdit
          :moduleId="currentProgram.name"
          :current="currentCheck"
          @refetch-data="getChecks"
          :isActiveSideBarEditCheck.sync="isActiveSideBarEditCheck"/>
...
import {
  BCard, BTable, BRow, BCol, BPagination, BDropdown, BDropdownItem, BDropdownForm, BBadge, BFormCheckbox, BOverlay, BModal, BImg,
} from 'bootstrap-vue';
...
export default {
  data() {
    return {
      ...
      isActiveSideBarViewCheck: false,
      isActiveSideBarEditCheck: false,
      pending: null,
      pendingOptions: null,
  ...
  computed: {
    ...
    fields() {
      return this.$store.getters['check/options'];
    },

    totalCount() {
      /* eslint no-underscore-dangle: ["error", { "allow": ["_meta"] }] */
      return this.$store.getters['check/list']?._meta.totalCount;
    },
    ...
    profiles() {
      return this.$store.getters['check/list']?.items;
    },
  ...
  watch: {
    currentProgram() {
      this.currentPage = 1;
      this.page = 1;
      this.getChecks(this.currentProgram?.name);
    },
  },
  ...
  methods: {
    ...
    openImages(a) {
      this.currentImages = JSON.parse(a);

      this.$refs['modal-swiper'].show();
    },
    ...
    showSideBarEditCheck(data) {
      this.currentCheck = data;

      this.isActiveSideBarEditCheck = true;
    },

    showSideBarViewCheck(data) {
      this.currentCheck = data;

      this.isActiveSideBarViewCheck = true;
    },
    ...
    async deleteCheck(id, moduleId) {
      this.pending = true;

      const confirm = await this.confirmText();

      if (confirm.value) {
        await this.$store.dispatch('check/DeleteCheck', {
          id,
          moduleId,
        });
    ...
    async setTableFields(field) {
      ...
      this.$store.commit('check/SET_OPTIONS', this.fields);
      ...
      await this.$store.dispatch('check/SetOption', data);
    ...
    async getChecks() { // все профили текущего промо
      this.pending = true;
      ...
      await this.$store.dispatch('check/GetChecks', o);

      this.pending = null;
    },

    async getOptions(moduleId) { // получить список полей для создания и редактирвания
      await this.$store.dispatch('check/GetOptions', {
        moduleId,
      });
    },
  },
