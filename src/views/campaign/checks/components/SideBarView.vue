<template>
  <b-sidebar
      width="40vw"
      id="sidebar-right"
      :visible="isActiveSideBarViewCheck"
      bg-variant="white"
      ...
      no-header
      right
      @change="(val) => $emit('update:is-active-side-bar-view-check', val)"
  >
...
export default {
  ...
  model: {
    prop: 'isActiveSideBarViewCheck',
    event: 'update:is-active-side-bar-view-check',
  },

  props: {
    isActiveSideBarViewCheck: {
      type: Boolean,
      required: true,
    },
  ...
  computed: {
    fields() {
      const a = this.$store.getters['check/options']?.map((item) => {
