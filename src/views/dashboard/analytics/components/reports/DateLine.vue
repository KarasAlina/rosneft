...
import {
  BRow, BCol, BTable, BPagination, BTd, BTr,
} from 'bootstrap-vue';
  ...
  methods: {
    ...
    async fetchData(obj) {
      ...
      keysColumnsFiltered.map((item) => {
        if (data.columns[item].type === 'bar') {
          const array = keys.map((key) => data.data[key][item]);
      ...
      keysColumnsFiltered.map((item, index) => {
        const b = JSON.parse(JSON.stringify(bar));

        const l = JSON.parse(JSON.stringify(line));

        if (data.columns[item].type === 'bar') {
          b.seriesName = data.columns[item].title;

          b.title.text = data.columns[item].title;

          b.axisBorder.color = this.chartOptions.colors[index];

          b.labels.style.colors = this.chartOptions.colors[index];

          b.title.style.color = this.chartOptions.colors[index];
        ...
        if (data.columns[item].type === 'line') {
          l.seriesName = data.columns[item].title;

          l.title.text = data.columns[item].title;

          l.axisBorder.color = this.chartOptions.colors[index];

          l.labels.style.colors = this.chartOptions.colors[index];

          l.title.style.color = this.chartOptions.colors[index];

          this.chartOptions.yaxis.push(l);
        }

        this.series.push({
          name: data.columns[item].title,
          type: data.columns[item].type === 'bar' ? 'column' : 'line',
          data: keys.map((key) => data.data[key][item]),
        });

        return null;
      });

      this.chartOptions.xaxis.categories = keys.map((item) => this.$date(item.split(' ')[0]).format('DD/MM/YY'));
